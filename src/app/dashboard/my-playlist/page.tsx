...
const MOODS: MoodCategory[] = [
  {
    ...
    name: "Feel-Good",
    icon: Smile,
    description: "Uplifting and warm stories",
  },
  {
    id: "intense",
    name: "Intense",
    icon: Zap,
    description: "High-stakes suspense and thrillers",
  },
  {
    id: "nostalgic",
    name: "Nostalgic",
    icon: History,
    description: "Timeless retro cinema",
  },
  {
    id: "heartbreak",
    name: "Emotional",
    icon: Heart,
    description: "Deep romances and touching dramas",
  },
  {
    id: "adrenaline",
    name: "Adrenaline",
    icon: Compass,
    description: "Action-packed cinematic spectacles",
  },
];
...
export default function MyPlaylistsPage() {
  ...
  const [detailsPlaylist, setDetailsPlaylist] = useState<PlaylistItem | null>(
    null,
  );
  ...
  const filteredPlaylists = selectedMood
    ? playlists.filter((pl) => {
        ...
        return (
          cleanTag.includes(cleanMood) ||
          pl.name.toLowerCase().includes(cleanMood)
        );
      })
    : playlists;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-0.5 border-b border-[#1A1A1A] pb-5">
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-xs md:text-sm text-zinc-500 font-medium max-w-2xl leading-relaxed">
              Curate and organize your cinema collections with shareable
              YouTube-style public links.
            </p>
          </div>
            ...
            {MOODS.map((mood) => {
              ...
              return (
                ...
                >
                  <MoodIcon
                    size={16}
                    className={isSelected ? "text-[#FF4C00]" : ""}
                  />
        ...
        <section className="flex flex-col gap-4">
          ...
          {loading ? (
            ...
            <EmptyState
              title={
                selectedMood
                  ? `No "${selectedMood}" Playlists`
                  : "No Playlists Yet"
              }
              description={
                selectedMood
                  ? `You don't have any playlists tagged with "${selectedMood}".`
                  : "Build your first custom playlist with your favorite films or randomize a theme to get started."
              }
              icon={Film}
              actionText={
                selectedMood ? "Show All Playlists" : "Create Playlist"
              }
              onActionClick={() =>
                selectedMood ? setSelectedMood(null) : handleOpenCreateModal()
              }
            />
          ) : (
