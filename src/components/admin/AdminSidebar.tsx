...
export default function AdminSidebar({
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-[#0A0A0A] text-white select-none relative font-sans overflow-visible">
      <div
        className={`flex items-center justify-between px-6 pt-6 pb-4 overflow-visible ${isCollapsed ? "justify-center px-2" : ""}`}
      >
        {!isCollapsed ? (
      ...
      <nav
        className={`flex-grow px-4 py-6 space-y-2.5 overflow-visible ${isCollapsed ? "px-2" : ""}`}
      >
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              >
                ...
                {!isCollapsed && (
                  <span className="transition-opacity duration-200 flex-grow">
                    {item.label}
                  </span>
                )}
      ...
      <div
        className={`p-4 border-t border-[#1A1A1A] relative ${isCollapsed ? "flex justify-center" : ""}`}
      >
          ...
          >
            ...
            {!isCollapsed && (
              <div className="flex flex-col min-w-0 pr-4">
                ...
                <span className="text-[10px] text-zinc-500 truncate font-semibold uppercase tracking-wider">
                  Super Admin
                </span>
              </div>
            )}
  ...
  return (
    <>
      <aside
        className={`hidden md:block h-screen bg-[#0A0A0A] shrink-0 sticky top-0 transition-all duration-300 ease-in-out overflow-visible z-30 ${
      ...
      {isOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex bg-black/60 backdrop-blur-sm transition-opacity duration-300">
          <div className="w-[260px] h-full shadow-2xl">{sidebarContent}</div>
